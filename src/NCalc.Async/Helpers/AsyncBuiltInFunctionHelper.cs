...
public static class AsyncBuiltInFunctionHelper
{
    public static async ValueTask<object?> EvaluateAsync(string functionName, AsyncExpression[] arguments, AsyncExpressionContext context, CancellationToken ct = default)
    {
        ...
        if (functionName.Equals("IEEERemainder", comparison))
        {
            ...
            return MathHelper.IEEERemainder(await arguments[0].EvaluateAsync(ct), await arguments[1].EvaluateAsync(ct), context);
        }
        if (functionName.Equals("Ln", comparison))
        {
            if (arguments.Length != 1)
                throw new NCalcEvaluationException("Ln() takes exactly 1 argument");
            return MathHelper.Ln(await arguments[0].EvaluateAsync(ct), context);
        }
        if (functionName.Equals("Log", comparison))
        {
            if (arguments.Length != 2)
                throw new NCalcEvaluationException("Log() takes exactly 2 arguments");
            return MathHelper.Log(await arguments[0].EvaluateAsync(ct), await arguments[1].EvaluateAsync(ct), context);
        }
        if (functionName.Equals("Log10", comparison))
        {
            if (arguments.Length != 1)
                throw new NCalcEvaluationException("Log10() takes exactly 1 argument");
            return MathHelper.Log10(await arguments[0].EvaluateAsync(ct), context);
        }
        if (functionName.Equals("Pow", comparison))
        {
            if (arguments.Length != 2)
                throw new NCalcEvaluationException("Pow() takes exactly 2 arguments");
            return MathHelper.Pow(await arguments[0].EvaluateAsync(ct), await arguments[1].EvaluateAsync(ct), context);
        }
        ...
        if (functionName.Equals("ifs", comparison))
        {
            ...
            foreach (var argument in arguments.Where((_, i) => i % 2 == 0))
            {
                ...
                if (index == arguments.Length - 1)
                    return await argument.EvaluateAsync(ct);

                var tf = Convert.ToBoolean(await argument.EvaluateAsync(ct), context.CultureInfo);
                if (tf)
                    return await arguments[index + 1].EvaluateAsync(ct);
            }
        ...
        if (functionName.Equals("if", comparison))
        {
            ...
            var cond = Convert.ToBoolean(await arguments[0].EvaluateAsync(ct), context.CultureInfo);
            return cond ? await arguments[1].EvaluateAsync(ct) : await arguments[2].EvaluateAsync(ct);
        }
        if (functionName.Equals("in", comparison))
        {
            if (arguments.Length < 2)
                throw new NCalcEvaluationException("in() takes at least 2 arguments");
            var parameter = await arguments[0].EvaluateAsync(ct);
            var evaluation = false;
            for (var i = 1; i < arguments.Length; i++)
            {
                if (TypeHelper.CompareUsingMostPreciseType(parameter, await arguments[i].EvaluateAsync(ct), context) != 0) continue;
                evaluation = true;
                break;
            }
