...
{
    [Test]
    public async Task ShouldGetParametersIssue103()
    {
        ...
        var parameters = expression.GetParameterNames(CancellationToken.None);
    ...
    [Test]
    public async Task ShouldGetParametersOneTimeIssue141()
    {
        var expression =
    ...
    [Test]
    public async Task ShouldGetParametersWithUnary()
    {
        var expression = new Expression("-0.68");
        var p = expression.GetParameterNames(CancellationToken.None);
    ...
    public async Task ShouldGetParameters(string formula, int expectedCount)
    {
    ...
    [Test]
    public async Task ShouldGetFunctions(string formula, int expectedCount)
    {
        var expression = new Expression(formula);
        var functions = expression.GetFunctionNames(CancellationToken.None);
        await Assert.That(functions.Count).IsEqualTo(expectedCount);
    }

    [Test]
    public async Task ShouldGetParametersInsideFunctionsIssue305()
    {
    ...
    [Test]
    public async Task ShouldGetFunctionsInsideFunctionsIssue305()
    {
