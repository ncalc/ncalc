...
public class EventHandlersTests
{
    ...
    public async Task ShouldEvaluateStringArgumentsInCustomFunctionWithIterateParametersIssue83()
    {
        ...
        e.EvaluateFunction += (name, args) =>
        {
            ...
            var firstParam = args.Parameters.Evaluate(0);
            var secondParam = args.Parameters.Evaluate(1);
        ...
        e.EvaluateFunction += (name, arg) =>
        {
            switch (name)
            {
                case "func1":
                    ...
                    break;
                case "func2":
                    arg.Result = 2 * Convert.ToDouble(arg.Parameters.Evaluate(0));
                    break;
                case "func3":
                    arg.Result = 3 * Convert.ToDouble(arg.Parameters.Evaluate(0));
                    break;
            }
        };
    ...
    public async Task ExpressionShouldEvaluateCustomFunctionsWithEffects()
    {
        ...
        e.EvaluateFunction += (name, args) =>
        {
            ...
            if (name == "Repeat")
            {
                var t = (int)args.Parameters.Evaluate(1) - 1;
                var r = (bool)args.Parameters.Evaluate(0);
                if (r && id != null)
                {
