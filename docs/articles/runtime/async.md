...
## Concurrent Binary Evaluation

By default, `EvaluateAsync()` evaluates the left and right operands of a binary expression sequentially. Enable
<xref:NCalc.ExpressionEvaluationOptions.ConcurrentBinaryAsyncEvaluation> to start both operands concurrently when they do
not require short-circuit evaluation. This can reduce latency when independent operands perform asynchronous I/O.
...
function arguments also retain their existing lazy behavior. Enabling concurrent evaluation can change operand execution
order, so custom callbacks and any state they access must be safe for concurrent use.
